...
export const ReportFormModal = ({
  ...
  const handleClose = (isSubmitted?: boolean) => {
    const hasContent = comment.trim().length > 0 || photos.length > 0;

    // Don't show confirmation if form was successfully submitted
    if (hasContent && !isSubmitted) {
      Alert.alert(
        t('report.confirmClose.title'),
        t('report.confirmClose.message'),
        [
          {
          ...
          {
            text: t('report.confirmClose.confirm'),
            style: 'destructive',
            onPress: () => onClose(),
          },
        ]
  ...
  const handleClosePress = () => handleClose(false);
